...
namespace Tutorial3.Tests
{
    public class EmpDeptSalgradeTests
    {
        ...
        public void ShouldReturnEmployeesFromChicago()
        {
            ...
            var chicagoDeptNos = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo);
        ...
        public void ShouldMatchEmployeeToSalaryGrade()
        {
            ...
            var result = emps.Join(grades.Where(s => emps.Any(e => e.Sal >= s.Losal && e.Sal <= s.Hisal)),
                    e => e.Sal,
                    s => s.Losal, // we join on salary ranges
        ...
        {
            var emps = Database.GetEmps();
            var result = emps.Where(e =>
            {
                var deptAvg = emps.Where(x => x.DeptNo == e.DeptNo).Average(x => x.Sal);
